...
#include <vector>
#include <stdio.h>
#include "./gridify_occaware-inl.h"
...
__global__ void gridify_occaware_sampling (
    ...
    int occ_to_coor_ind_b = blockIdx.x * blockDim.x + threadIdx.x; // index of gpu thread
    int occ_to_coor_ind = occ_to_coor_ind_b / B;
    int i_batch = occ_to_coor_ind_b % B;
    if (i_batch >= B || occ_to_coor_ind < max_o || occ_to_coor_ind >= out_actual_centnum[i_batch]) { return; }
    ...
    while (counter < 4) {
        ...
        int occ_lst[occ_size * 2], occ_number_cell=0;
        int d_coor_add=0, h_coor_add=0, w_coor_add=0;
        ...
        for (int nei_idx = 0; nei_idx < size; nei_idx++) {
            ...
            if (d_coor >= 0 && d_coor < d_grid_size[2] && h_coor >= 0 &&
                ...
                if (occ_number_cell == 1){
                    removedocc += 0.7;
                    if (coor_to_voxelidx[coor_indx_b_o]  > -1) removedocc += 0.3;
                }
            }
            ...
            if (d_coor >= 0 && d_coor < d_grid_size[2] && h_coor >= 0 &&
                ...
                if (occ_number_cell == 0){
                    addocc += 0.7;
                    if (coor_to_voxelidx[coor_indx_b_o]  > -1) addocc += 0.3;
                }
            }
        }
...
//                printf("redo even addocc %f > removedocc %f \n", addocc, removedocc);
        ...
        } else if (counter > 0 && addocc > removedocc) {
            if (atomicCAS(&voxelidx_to_coor[i_batch * max_o + insrtidx], coor_o, coor_indx) == coor_o){
                for (int i = 0; i < occ_size * 0.5; i++){
                    atomicAdd(coor_to_occ + occ_lst[i*2], -1);
                    atomicAdd(coor_to_occ + occ_lst[i*2 + occ_size], 1);
...
template<typename Dtype>
__global__ void gridify_kernel_build_index_occaware(
        int* out_nebidx, Dtype* out_nebidxmsk, Dtype* out_cent,
        Dtype* out_centmsk, int* out_actual_centnum,
        const Dtype* in_data, const int* in_actual_numpoints,
        const int B,
...
template<typename Dtype>
__global__ void gridify_kernel_query_neighs_occaware(
        int* out_nebidx, Dtype* out_nebidxmsk, Dtype* out_cent,
        Dtype* out_centmsk, int* out_actual_centnum,
        const Dtype* in_data, const int* in_actual_numpoints,
        const int B,
...
template<typename Dtype>
inline void GridifyForward_occaware(Tensor<gpu, 3, int> &nebidx, // B * O * P
                           ...
                           const mxnet::op::GridifyParam_occaware &param
) {
...
//    printf("dimGrid: %d ; dimBlock %d ;", gridSize, cuda::kMaxThreadsPerBlock);
    cuda::CheckLaunchParam(dimGrid, dimBlock, "GridifyForward_occaware");
...
//    printf("seconds: %lu", seconds);
    cuda::gridify_kernel_build_index_occaware<Dtype><<<dimGrid, dimBlock>>>(
    ...
    MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::gridify_kernel_build_index_occaware);
...
//    const int gridSize_s = (B * 1 + cuda::kMaxThreadsPerBlock - 1) / cuda::kMaxThreadsPerBlock;
    ...
    cuda::gridify_occaware_sampling<Dtype><<<s_dimGrid, s_dimBlock>>>(
            out_nebidx, out_nebidxmsk, out_cent, out_centmsk, out_actual_centnum, in_data, in_actual_numpoints,
                    B, N, O, P, param.kernel_size, param.stride, param.loc, d_coord_shift, d_voxel_size, d_grid_size,
                    grid_size_vol, size, d_coor_to_voxelidx, d_voxelidx_to_coor, d_coor_to_pntidx, d_coor_to_locxyzw, d_coor_counter,
                    seconds, d_coor_to_occ, d_occ_to_coor);
    MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::gridify_occaware_sampling);
    ...
    cuda::gridify_kernel_query_neighs_occaware<Dtype><<<o_dimGrid, o_dimBlock>>>(
            out_nebidx, out_nebidxmsk, out_cent, out_centmsk, out_actual_centnum, in_data, in_actual_numpoints,
            B, N, O, P, param.kernel_size, param.stride, param.loc, d_coord_shift, d_voxel_size, d_grid_size,
    ...
    MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::gridify_kernel_query_neighs_occaware);
...
namespace mxnet {
    namespace op {
        ...
        template<>
        Operator *CreateOp<gpu>(GridifyParam_occaware param, int dtype) {
            Operator *op = NULL;
            MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
                    op = new GridifyOp_occaware<gpu, DType>(param);
            });
            return op;
        }
