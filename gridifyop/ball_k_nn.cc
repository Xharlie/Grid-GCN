...
DMLC_REGISTER_PARAMETER(BallKNNParam);

NNVM_REGISTER_OP(_contrib_BallKNN)
.describe("Ball KNN foward.")
.set_num_inputs(4)
.set_num_outputs(1)
.set_attr_parser(ParamParser<BallKNNParam>)
...
.set_attr<FCompute>("FCompute<cpu>", BallKNNForward<cpu>)
...
.add_arguments(BallKNNParam::__FIELDS__());
