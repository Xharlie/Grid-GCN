 ...
 * \file gridify_occaware-inl.h
 * \brief
 * \author Qiangeng Xu
*/
#ifndef MXNET_OPERATOR_GRIDIFY_occaware_INL_H_
#define MXNET_OPERATOR_GRIDIFY_occaware_INL_H_
...
namespace mxnet {
    namespace op {
        namespace gridify_occaware {
            enum gridifyInput {
                kData, kActualnum
            };
            enum gridifyOutputs_occaware {
                kNeighborsB, kNeighborsMaskB, kCentsB, kCentsMaskB, kCentsNumB
            };
            enum gridifyResource_occaware {
                kTempSpace
            };
        } // namespace gridifier


        struct GridifyParam_occaware : public dmlc::Parameter<GridifyParam_occaware> {
//    width, height, depth from birdview
            ...
            DMLC_DECLARE_PARAMETER(GridifyParam_occaware) {
        ...
        std::ostream &operator<<(std::ostream &os, const GridifyParam_occaware &m);

        template<typename xpu, typename DType>
        class GridifyOp_occaware : public Operator {

        public:
            explicit GridifyOp_occaware(GridifyParam_occaware param) {
                this->param_ = param;
            }
            ...
            virtual void Forward(const OpContext &ctx,
                ...
                Tensor<xpu, 3, DType> data = in_data[gridify_occaware::kData].get<xpu, 3, DType>(s); // B * N * 4
                Tensor<xpu, 2, int> actual_numpoints = in_data[gridify_occaware::kActualnum].get<xpu, 2, int>(s); // B * 1
                Tensor<xpu, 3, int> nebidx = out_data[gridify_occaware::kNeighborsB].get<xpu, 3, int>(s); // B * O * P
                Tensor<xpu, 3, DType> nebidxmsk = out_data[gridify_occaware::kNeighborsMaskB].get<xpu, 3, DType>(s); // B * O * P
                Tensor<xpu, 3, DType> cent = out_data[gridify_occaware::kCentsB].get<xpu, 3, DType>(s); // B * O * 4
                Tensor<xpu, 2, DType> centmsk = out_data[gridify_occaware::kCentsMaskB].get<xpu, 2, DType>(s); // B * O
                Tensor<xpu, 2, int> actual_centnum = out_data[gridify_occaware::kCentsNumB].get<xpu, 2, int>(s); // B * 1
    ...
    CHECK_EQ(err, cudaSuccess) << "Name: " << "MemInit, " << " ErrStr:" << cudaGetErrorString(err);
                ...
                GridifyForward_occaware(nebidx, nebidxmsk, cent, centmsk, actual_centnum,
                               data, actual_numpoints, param_);
            }

        private:
            GridifyParam_occaware param_;
...
// Declare Factory function, used for dispatch specialization
        ...
        Operator *CreateOp(GridifyParam_occaware param, int dtype);

#if DMLC_USE_CXX11
        class GridifyProp_occaware : public OperatorProperty {
          public:
          ...
          int NumVisibleOutputs() const override {
            ...
            bool InferShape(std::vector<TShape> *in_shape,
              ...
              // data: [batch_size, N, C]
              TShape dshape = in_shape->at(gridify_occaware::kData);
              CHECK_EQ(dshape.ndim(), 3U) << "data should be a 3D tensor";

              // Actualnum: [batch_size]
              TShape ashape = in_shape->at(gridify_occaware::kActualnum);
...
//                Tensor<xpu, 1, DType> actual_centnum = out_data[gridify::kCentsNumB].get<xpu, 1, DType>(s); // B
            ...
            OperatorProperty* Copy() const override {
              GridifyProp_occaware* sym = new GridifyProp_occaware();
              sym->param_ = this->param_;
              return sym;
            }

            std::string TypeString() const override {
              return "Gridify_occaware";
            }
...
#endif  // MXNET_OPERATOR_GRIDIFY_occaware_INL_H_
