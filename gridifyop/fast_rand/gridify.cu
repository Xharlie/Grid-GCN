...
__global__ void gridify_kernel_build_index(
    ...
    int nei_idx = (threadindex + size / 2) % size;
    ...
    if (i_pt < in_actual_numpoints[i_batch]) {
        ...
        if (d_coor >= 0 && d_coor < d_grid_size[2] && h_coor >= 0 && h_coor < d_grid_size[1] && w_coor >= 0 &&
            ...
            if (grid_pntidx < P) {
                ...
                curand_init(threadindex, 0, 0, &state);
...
__global__ void gridify_kernel_query_neighs(
    ...
    if (i_ogrid < out_actual_centnum[i_batch]) {
        int d_coor, h_coor, w_coor, coor_indx_b, in_data_eleweight, idx, initID=0, idx_b_data;
        float xsum = 0.0, ysum = 0.0, zsum = 0.0, weightsum=0.0, countweightsum = 0;
        d_coor = voxelidx_to_coor[index * 3 + 2];
        h_coor = voxelidx_to_coor[index * 3 + 1];
        w_coor = voxelidx_to_coor[index * 3];
        coor_indx_b = i_batch * grid_size_vol + d_coor * (d_grid_size[0] * d_grid_size[1])
                      + h_coor * d_grid_size[0] + w_coor;
        int index_data = index * data_ndim;
        int index_P = index * P;
        int countlimit = coor_counter[coor_indx_b];
        for (int j = 0; j < P; j++){
            if (j < countlimit){
                idx = coor_to_pntidx[coor_indx_b * P + j];
                idx_b_data = (idx + N * i_batch) * data_ndim;
                if (j == 0){initID = idx;}
                out_nebidx[index_P + j] = idx;
                out_nebidxmsk[index_P + j] = 1.0;
                in_data_eleweight = in_data[idx_b_data + 3];
                if (loc == 0){
                    xsum += in_data[idx_b_data] * in_data_eleweight;
                    ysum += in_data[idx_b_data + 1] * in_data_eleweight;
                    zsum += in_data[idx_b_data + 2] * in_data_eleweight;
                    countweightsum += in_data_eleweight;
                }
                weightsum += in_data_eleweight;
            } else {
                out_nebidx[index_P + j] = initID;
            }
        }
        if(loc==1){
            int coor_indx_b_data = coor_indx_b * data_ndim;
...
//            printf("xsum:%f,ysum:%f,zsum:%f,w:%f \n",xsum,ysum,zsum,countweightsum);
        }
        ...
        out_cent[index_data + 3] = weightsum;
    }
}
}  // namespace cuda
...
//    }
    ...
    int *d_coor_to_voxelidx, *d_voxelidx_to_coor, *d_coor_to_pntidx, *d_coor_counter;
    ...
    cudaMalloc(&d_voxelidx_to_coor, B * O * ndim * sizeof(int));
