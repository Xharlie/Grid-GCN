 ...
 * \author Qiangeng Xu
*/
#ifndef MXNET_OPERATOR_GRIDIFY_INL_H_
#define MXNET_OPERATOR_GRIDIFY_INL_H_
...
namespace mxnet {
    namespace op {
        namespace gridify {
            ...
            enum gridifyResource {
                kTempSpace
            };
        } // namespace gridifier


        struct GridifyParam : public dmlc::Parameter<GridifyParam> {
//    width, height, depth from birdview
            ...
            DMLC_DECLARE_PARAMETER(GridifyParam) {
        ...
        std::ostream &operator<<(std::ostream &os, const GridifyParam &m);

        template<typename xpu, typename DType>
        class GridifyOp : public Operator {

        public:
            explicit GridifyOp(GridifyParam param) {
                this->param_ = param;
            }
            ...
            virtual void Forward(const OpContext &ctx,
                ...
                Tensor<xpu, 3, DType> data = in_data[gridify::kData].get<xpu, 3, DType>(s); // B * N * 4
                Tensor<xpu, 2, int> actual_numpoints = in_data[gridify::kActualnum].get<xpu, 2, int>(s); // B * 1
                Tensor<xpu, 3, int> nebidx = out_data[gridify::kNeighborsB].get<xpu, 3, int>(s); // B * O * P
                ...
                Tensor<xpu, 2, int> actual_centnum = out_data[gridify::kCentsNumB].get<xpu, 2, int>(s); // B * 1
    ...
    CHECK_EQ(err, cudaSuccess) << "Name: " << "MemInit, " << " ErrStr:" << cudaGetErrorString(err);
                ...
                GridifyForward(nebidx, nebidxmsk, cent, centmsk, actual_centnum,
...
// Declare Factory function, used for dispatch specialization
        ...
        Operator *CreateOp(GridifyParam param, int dtype);

#if DMLC_USE_CXX11
        class GridifyProp : public OperatorProperty {
          public:
          ...
          int NumVisibleOutputs() const override {
            ...
            bool InferShape(std::vector<TShape> *in_shape,
              ...
              // data: [batch_size, N, C]
              TShape dshape = in_shape->at(gridify::kData);
              CHECK_EQ(dshape.ndim(), 3U) << "data should be a 3D tensor";

              // Actualnum: [batch_size]
              TShape ashape = in_shape->at(gridify::kActualnum);
...
//                Tensor<xpu, 1, DType> actual_centnum = out_data[gridify::kCentsNumB].get<xpu, 1, DType>(s); // B
            ...
            OperatorProperty* Copy() const override {
              GridifyProp* sym = new GridifyProp();
...
#endif  // MXNET_OPERATOR_GRIDIFY_INL_H_
