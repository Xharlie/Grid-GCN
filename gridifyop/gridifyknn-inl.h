 ...
 * \file gridifyknn-inl.h
 * \brief
 * \author Qiangeng Xu
*/
#ifndef MXNET_OPERATOR_GRIDIFYKNN_INL_H_
#define MXNET_OPERATOR_GRIDIFYKNN_INL_H_
...
namespace mxnet {
    namespace op {
        namespace gridifyKNN {
            enum gridifyKNNInput {
                kData, kActualnum
            };
            enum gridifyKNNOutputs {
                kNeighborsB, kNeighborsMaskB, kCentsB, kCentsMaskB, kCentsNumB
            };
            enum gridifyKNNResource {
                kTempSpace
            };
        } // namespace gridifier


        struct GridifyKNNParam : public dmlc::Parameter<GridifyKNNParam> {
//    width, height, depth from birdview
            ...
            DMLC_DECLARE_PARAMETER(GridifyKNNParam) {
        ...
        std::ostream &operator<<(std::ostream &os, const GridifyKNNParam &m);

        template<typename xpu, typename DType>
        class GridifyKNNOp : public Operator {

        public:
            explicit GridifyKNNOp(GridifyKNNParam param) {
                this->param_ = param;
            }
            ...
            virtual void Forward(const OpContext &ctx,
                ...
                Tensor<xpu, 3, DType> data = in_data[gridifyKNN::kData].get<xpu, 3, DType>(s); // B * N * 4
                Tensor<xpu, 2, int> actual_numpoints = in_data[gridifyKNN::kActualnum].get<xpu, 2, int>(s); // B * 1
                Tensor<xpu, 3, int> nebidx = out_data[gridifyKNN::kNeighborsB].get<xpu, 3, int>(s); // B * O * P
                Tensor<xpu, 3, DType> nebidxmsk = out_data[gridifyKNN::kNeighborsMaskB].get<xpu, 3, DType>(s); // B * O * P
                Tensor<xpu, 3, DType> cent = out_data[gridifyKNN::kCentsB].get<xpu, 3, DType>(s); // B * O * 4
                Tensor<xpu, 2, DType> centmsk = out_data[gridifyKNN::kCentsMaskB].get<xpu, 2, DType>(s); // B * O
                Tensor<xpu, 2, int> actual_centnum = out_data[gridifyKNN::kCentsNumB].get<xpu, 2, int>(s); // B * 1
    ...
    CHECK_EQ(err, cudaSuccess) << "Name: " << "MemInit, " << " ErrStr:" << cudaGetErrorString(err);
                ...
                GridifyKNNForward(nebidx, nebidxmsk, cent, centmsk, actual_centnum,
                               data, actual_numpoints, param_);
            }

        private:
            GridifyKNNParam param_;
...
// Declare Factory function, used for dispatch specialization
        ...
        Operator *CreateOp(GridifyKNNParam param, int dtype);

#if DMLC_USE_CXX11
        class GridifyKNNProp : public OperatorProperty {
          public:
          ...
          int NumVisibleOutputs() const override {
            ...
            bool InferShape(std::vector<TShape> *in_shape,
              ...
              // data: [batch_size, N, C]
              TShape dshape = in_shape->at(gridifyKNN::kData);
              CHECK_EQ(dshape.ndim(), 3U) << "data should be a 3D tensor";

              // Actualnum: [batch_size]
              TShape ashape = in_shape->at(gridifyKNN::kActualnum);
...
//                Tensor<xpu, 1, DType> actual_centnum = out_data[gridify::kCentsNumB].get<xpu, 1, DType>(s); // B
            ...
            OperatorProperty* Copy() const override {
              GridifyKNNProp* sym = new GridifyKNNProp();
...
#endif  // MXNET_OPERATOR_GRIDIFYKNN_INL_H_
