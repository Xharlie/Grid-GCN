...
namespace mshadow {
    ...
    static __device__  half::half_t atomicAdd(half::half_t  * address, half::half_t val ) {
    ...
    static __device__ half::half_t cas(half::half_t * addr, half::half_t compare, half::half_t val) {
    // NOT IMPLEMENTED YET!
        return 0;
    }
...
__global__ void gridify_occaware_sampling_s (
    ...
    int i_batch = blockIdx.x * blockDim.x + threadIdx.x; // index of gpu thread
    if (i_batch >= B) { return; }
    for (int occ_to_coor_ind = max_o; occ_to_coor_ind < out_actual_centnum[i_batch]; occ_to_coor_ind++){
        int occ_to_coor_ind_b = occ_to_coor_ind + i_batch * grid_size_vol;
        int coor_indx = occ_to_coor[occ_to_coor_ind_b];
        curandState state;
        int coor[ndim];
        coor[2] = coor_indx / (d_grid_size[0] * d_grid_size[1]);
        coor[1]  = (coor_indx - coor[2]* (d_grid_size[0] * d_grid_size[1])) / d_grid_size[0];
        coor[0]  = coor_indx - coor[2]* (d_grid_size[0] * d_grid_size[1]) - coor[1] * d_grid_size[0];

        // increase the counter, return old counter
        int d_coor, h_coor, w_coor, coor_indx_b_o;
    //        int insrtidx = ceilf(curand_uniform(&state) * (max_o+1)) - 1;
        int counter = 1;
        while (counter < 11) {
            curand_init(occ_to_coor_ind_b*counter+2*seconds, 0, 0, &state);
            int insrtidx = ceilf(curand_uniform(&state) * (max_o)) - 1;
//            printf("%d,%d / %d;",insrtidx,occ_to_coor_ind, out_actual_centnum[i_batch]);
            if (insrtidx>=max_o) continue;
            float removedocc=0, addocc=0;
            int occ_lst[occ_size * 2];
            int d_coor_add=0, h_coor_add=0, w_coor_add=0, occ_number_cell=0;
            int coor_o = voxelidx_to_coor[i_batch * max_o + insrtidx];
            int coor2 = coor_o / (d_grid_size[0] * d_grid_size[1]);
            int coor1 = (coor_o - coor2* (d_grid_size[0] * d_grid_size[1])) / d_grid_size[0];
            int coor0 = coor_o - coor2* (d_grid_size[0] * d_grid_size[1]) - coor1 * d_grid_size[0];
            for (int nei_idx = 0; nei_idx < size; nei_idx++) {
                d_coor_add = nei_idx / (kernel_size * kernel_size) - (kernel_size - 1) / 2;
                h_coor_add = (nei_idx % (kernel_size * kernel_size)) / kernel_size - (kernel_size - 1) / 2;
                w_coor_add = nei_idx % kernel_size - (kernel_size - 1) / 2;

                d_coor = d_coor_add + coor2;
                h_coor = h_coor_add + coor1;
                w_coor = w_coor_add + coor0;
                if (d_coor >= 0 && d_coor < d_grid_size[2] && h_coor >= 0 &&
                    h_coor < d_grid_size[1] && w_coor >= 0 && w_coor < d_grid_size[0]) {
                    coor_indx_b_o = i_batch * grid_size_vol + d_coor * (d_grid_size[0] * d_grid_size[1])
                                    + h_coor * d_grid_size[0] + w_coor;
                    occ_lst[nei_idx * 2] = coor_indx_b_o;
                    occ_number_cell = coor_to_occ[coor_indx_b_o];
                    occ_lst[nei_idx * 2 + 1] = occ_number_cell;
                    if (occ_number_cell == 1){
                        removedocc += 0.9;
                        if (coor_to_voxelidx[coor_indx_b_o]  > -1) removedocc += 0.1;
                    }
                }
                d_coor = d_coor_add + coor[2];
                h_coor = h_coor_add + coor[1];
                w_coor = w_coor_add + coor[0];
                if (d_coor >= 0 && d_coor < d_grid_size[2] && h_coor >= 0 &&
                    h_coor < d_grid_size[1] && w_coor >= 0 && w_coor < d_grid_size[0]) {
                    coor_indx_b_o = i_batch * grid_size_vol + d_coor * (d_grid_size[0] * d_grid_size[1])
                                    + h_coor * d_grid_size[0] + w_coor;
                    occ_lst[occ_size + nei_idx * 2] = coor_indx_b_o;
                    occ_number_cell = coor_to_occ[coor_indx_b_o];
                    occ_lst[occ_size + nei_idx * 2 + 1] = occ_number_cell;
    //                    addocc += 1 / occ_number_cell;
    //                        printf("remove: occ cell reverse %f ;", 1 / occ_number_cell);
                    if (occ_number_cell == 0){
                        addocc += 0.9;
                        if (coor_to_voxelidx[coor_indx_b_o]  > -1) addocc += 0.1;
                    }

                }
            }
            if (addocc > removedocc + 3) {
                if (atomicCAS(&voxelidx_to_coor[i_batch * max_o + insrtidx], coor_o, coor_indx) == coor_o) {
                    for (int i = 0; i < occ_size * 0.5; i++) {
                        atomicAdd(coor_to_occ + occ_lst[i * 2], -1);
                        atomicAdd(coor_to_occ + occ_lst[i * 2 + occ_size], 1);
                    }
//                printf("addocc %f > removedocc %f \n", addocc, removedocc);
                    counter = 11;
                } else {
                    counter++;
//                printf("redo even addocc %f > removedocc %f \n", addocc, removedocc);
                }
            } else if (counter > 5 && addocc > removedocc) {
                if (atomicCAS(&voxelidx_to_coor[i_batch * max_o + insrtidx], coor_o, coor_indx) == coor_o){
                    for (int i = 0; i < occ_size * 0.5; i++){
                        atomicAdd(coor_to_occ + occ_lst[i*2], -1);
                        atomicAdd(coor_to_occ + occ_lst[i*2 + occ_size], 1);
...
template<typename Dtype>
__global__ void gridify_kernel_build_index_occaware_s(
        int* out_nebidx, Dtype* out_nebidxmsk, Dtype* out_cent,
        Dtype* out_centmsk, int* out_actual_centnum,
        const Dtype* in_data, const int* in_actual_numpoints,
        const int B,
    ...
    if (i_pt < in_actual_numpoints[i_batch]) {
        ...
        if (voxel_idx == -1) {  // found an empty voxel
            ...
            occ_to_coor[tmp + i_batch * grid_size_vol] = coor_indx;
            if (tmp < max_o) {
                int coor_indx_b_o, d_coor, h_coor, w_coor;
                ...
                // add coor_to_occ
                for (int nei_idx = 0; nei_idx < size; nei_idx++) {
                    d_coor = nei_idx / (kernel_size * kernel_size) - (kernel_size - 1) / 2 + coor[2];
                    h_coor = (nei_idx % (kernel_size * kernel_size)) / kernel_size - (kernel_size - 1) / 2 + coor[1];
                    w_coor = nei_idx % kernel_size - (kernel_size - 1) / 2 + coor[0];
                    if (d_coor >= 0 && d_coor < d_grid_size[2] && h_coor >= 0 &&
                        h_coor < d_grid_size[1] && w_coor >= 0 && w_coor < d_grid_size[0]) {
                        coor_indx_b_o = i_batch * grid_size_vol + d_coor * (d_grid_size[0] * d_grid_size[1])
                                        + h_coor * d_grid_size[0] + w_coor;
                        atomicAdd(coor_to_occ + coor_indx_b_o, 1);
                     }
                }
            }
//            if (old_voxel_num == -1) {
...
template<typename Dtype>
__global__ void gridify_kernel_query_neighs_occaware_s(
        int* out_nebidx, Dtype* out_nebidxmsk, Dtype* out_cent,
        Dtype* out_centmsk, int* out_actual_centnum,
        const Dtype* in_data, const int* in_actual_numpoints,
        const int B,
    ...
    if (out_actual_centnum[i_batch] > max_o) out_actual_centnum[i_batch] = max_o;
...
template<typename Dtype>
inline void GridifyForward_occaware_s(Tensor<gpu, 3, int> &nebidx, // B * O * P
...
//    }
    ...
    int *d_coor_to_voxelidx, *d_voxelidx_to_coor, *d_coor_to_pntidx, *d_coor_counter, *d_voxelidx_counter, *d_coor_to_occ, *d_occ_to_coor;
    ...
    cudaMalloc(&d_coor_to_occ, B * grid_size_vol * sizeof(int));
    cudaMalloc(&d_occ_to_coor, B * grid_size_vol * sizeof(int));
    ...
    cudaMemset(d_coor_to_occ,  0, B * grid_size_vol * sizeof(int));
    cudaMemset(d_occ_to_coor,  0, B * grid_size_vol * sizeof(int));
...
//    printf("dimGrid: %d ; dimBlock %d ;", gridSize, cuda::kMaxThreadsPerBlock);
    cuda::CheckLaunchParam(dimGrid, dimBlock, "GridifyForward_occaware_s");
...
//    printf("seconds: %lu", seconds);
    cuda::gridify_kernel_build_index_occaware_s<Dtype><<<dimGrid, dimBlock>>>(
            out_nebidx, out_nebidxmsk, out_cent, out_centmsk, out_actual_centnum, in_data, in_actual_numpoints,
                    ...
                    seconds, d_coor_to_occ, d_occ_to_coor);
    MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::gridify_kernel_build_index_occaware_s);

    const int gridSize_s = (B * 1 + cuda::kMaxThreadsPerBlock - 1) / cuda::kMaxThreadsPerBlock;
//    const int gridSize_s = (B * grid_size_vol + cuda::kMaxThreadsPerBlock - 1) / cuda::kMaxThreadsPerBlock;
    dim3 s_dimGrid(gridSize_s);
    dim3 s_dimBlock(cuda::kMaxThreadsPerBlock);

    cuda::gridify_occaware_sampling_s<Dtype><<<s_dimGrid, s_dimBlock>>>(
            out_nebidx, out_nebidxmsk, out_cent, out_centmsk, out_actual_centnum, in_data, in_actual_numpoints,
                    B, N, O, P, param.kernel_size, param.stride, param.loc, d_coord_shift, d_voxel_size, d_grid_size,
                    grid_size_vol, size, d_coor_to_voxelidx, d_voxelidx_to_coor, d_coor_to_pntidx, d_coor_to_locxyzw, d_coor_counter,
                    seconds, d_coor_to_occ, d_occ_to_coor);
    MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::gridify_occaware_sampling_s);
    ...
    cuda::gridify_kernel_query_neighs_occaware_s<Dtype><<<o_dimGrid, o_dimBlock>>>(
            out_nebidx, out_nebidxmsk, out_cent, out_centmsk, out_actual_centnum, in_data, in_actual_numpoints,
            B, N, O, P, param.kernel_size, param.stride, param.loc, d_coord_shift, d_voxel_size, d_grid_size,
    ...
    MSHADOW_CUDA_POST_KERNEL_CHECK(cuda::gridify_kernel_query_neighs_occaware_s);
    ...
    cudaFree(d_occ_to_coor);
    cudaFree(d_coor_to_occ);
}
...
namespace mxnet {
    namespace op {
        ...
        template<>
        Operator *CreateOp<gpu>(GridifyParam_occaware_s param, int dtype) {
            Operator *op = NULL;
            MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
                    op = new GridifyOp_occaware_s<gpu, DType>(param);
            });
            return op;
        }
