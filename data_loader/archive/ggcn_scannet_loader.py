...
class BaseScanNetLoader(mx.io.DataIter):
    def __init__(self, root, batch_size=32, npoints=8192, split='train', shuffle=False, normalize=False,
                 augment_data=0, dropout_ratio=0, use_weights=False, use_cache=False, cache_file=''):
        self._augment_batch_data = [self._augment_batch_data_l0, self._augment_batch_data_l1,
                                    self._augment_batch_data_l2, self._augment_batch_data_l3,
                                    self._augment_batch_data_l4, self._augment_batch_data_l5]
        self.root = root
        ...
        if sys.version_info[0] == 3:
            ...
            with open(os.path.join(self.root, 'scannet_{}.pickle'.format(self.split))) as f:
    ...
    def provide_data(self):
        ...
        for i in range(len(configs['max_o_grid_lst'])):
            if configs["agg"] in ["max", "max_pooling"] and configs["neigh_fetch"] == 0:
                if configs["up_type"] == "inter" or configs["up_neigh_fetch"]:
                    data_shapes += [('neighbors_arr' + str(i),
                                    (self.batch_size, configs['max_o_grid_lst'][i], configs['max_p_grid_lst'][i]), 'int32'),
                                    ('centers_arr' + str(i), (self.batch_size, configs['max_o_grid_lst'][i], 4)),
                                    ('centers_mask' + str(i), (self.batch_size, configs['max_o_grid_lst'][i]))]
                else:
                    if configs["up_agg"] in ["max", "max_pooling"] and configs["up_neigh_fetch"] == 0:
                        data_shapes += [('neighbors_arr' + str(i),
                                     (self.batch_size, configs['max_o_grid_lst'][i], configs['max_p_grid_lst'][i]), 'int32'),
                                    ('neighbors_arr_up' + str(i),
                                     (self.batch_size, configs['up_max_o_grid_lst'][i], configs['up_max_p_grid_lst'][i]), 'int32'),
                                    ('centers_arr' + str(i), (self.batch_size, configs['max_o_grid_lst'][i], 4)),
                                    ('centers_mask' + str(i), (self.batch_size, configs['max_o_grid_lst'][i]))]
                    else:
                        data_shapes += [('neighbors_arr' + str(i),
                                         (self.batch_size, configs['max_o_grid_lst'][i], configs['max_p_grid_lst'][i]),
                                         'int32'),
                                        ('neighbors_arr_up' + str(i),
                                         (self.batch_size, configs['up_max_o_grid_lst'][i],
                                          configs['up_max_p_grid_lst'][i]), 'int32'),
                ...
                if configs["up_type"] == "inter":
                    ...
                    if configs["up_agg"] in ["max", "max_pooling"] and configs["neigh_fetch"] == 0:
                        data_shapes += [('neighbors_arr' + str(i),
                                         (self.batch_size, configs['max_o_grid_lst'][i], configs['max_p_grid_lst'][i]), 'int32'),
                                        ('neighbors_arr_up' + str(i),
                                         (self.batch_size, configs['up_max_o_grid_lst'][i],
                        ...
                        data_shapes += [('neighbors_arr' + str(i),
                                 ...
                                 (self.batch_size, configs['max_o_grid_lst'][i], configs['max_p_grid_lst'][i])),
                                 ('neighbors_mask_up' + str(i),
                                 (self.batch_size, configs['up_max_o_grid_lst'][i], configs['up_max_p_grid_lst'][i])),
                                 ('centers_arr' + str(i), (self.batch_size, configs['max_o_grid_lst'][i], 4)),
                                 ('centers_mask' + str(i), (self.batch_size, configs['max_o_grid_lst'][i]))]
        return data_shapes
    ...
    def _augment_batch_data_l0(self, batch_data, batch_label):
    ...
    def _augment_batch_data_l4(self, batch_data, batch_label):
        # batch_data = utils.rotate_point_cloud_z(batch_data)
        # batch_data = utils.rotate_perturbation_point_cloud(batch_data)
        #batch_data = utils.random_scale_point_cloud(batch_data)
        #batch_data = utils.shift_point_cloud(batch_data)
    ...
    def _augment_batch_data_l5(self, batch_data, batch_label):
        batch_data = utils.rotate_point_cloud_z(batch_data)
        batch_data = utils.rotate_perturbation_point_cloud(batch_data)
        #batch_data = utils.random_scale_point_cloud(batch_data)
        #batch_data = utils.shift_point_cloud(batch_data)
...
class ScanNetLoader(BaseScanNetLoader):
    ...
    def __init__(self, root, batch_size=32, npoints=8192, split='train', shuffle=False, normalize=False,
                 augment_data=0, dropout_ratio=0, use_weights=False, use_cache=False, cache_file=''):
        super(ScanNetLoader, self).__init__(root, batch_size, npoints, split, shuffle, normalize, augment_data,
                                            dropout_ratio, use_weights, use_cache, cache_file)
    ...
    def _next_batch(self):
        if not self.use_cache or self.first_run:
            if self._has_next_batch():
                ...
                self.batch_idx += 1
                if self.use_weights:
                    batch_data_lst = [mx.ndarray.array(batch_data), mx.ndarray.array(batch_weight)]
                else:
                ...
                neighbors_arr_lst, neighbors_mask_lst, neighbors_arr_up_lst, neighbors_mask_up_lst,\
                    ...
                    gridify.assemble_tensor(batch_data, voxel_size_lst=configs['voxel_size_lst'],
                        grid_size_lst=configs['grid_size_lst'],
                        lidar_coord=configs['lidar_coord'], max_p_grid_lst=configs['max_p_grid_lst'],
                        max_o_grid_lst=configs['max_o_grid_lst'],
                        kernel_size_lst=configs['kernel_size_lst'], stride_lst=configs['stride_lst'],
                        single_padding_lst=configs['single_padding_lst'], para=configs["hash"] == "para",
                        allow_sub=True, loc_weight=configs["loc_weight"], loc_within=configs["loc_within"],
                        up_voxel_size_lst=configs['up_voxel_size_lst'], up_max_p_grid_lst=configs['up_max_p_grid_lst'],
                        up_max_o_grid_lst=configs['up_max_o_grid_lst'], up_grid_size_lst=configs['up_grid_size_lst'],
                        up_kernel_size_lst=configs['up_kernel_size_lst'], up_stride_lst=configs['up_stride_lst'])
                for i in range(len(neighbors_arr_lst)):
                    if configs["agg"] in ["max", "max_pooling"] and configs["neigh_fetch"] == 0:
                        if configs["up_type"] == "inter" or configs["up_neigh_fetch"]:
                            ...
                            if configs["up_agg"] in ["max", "max_pooling"] and configs["up_neigh_fetch"] == 0:
                                batch_data_lst += [mx.ndarray.array(neighbors_arr_lst[i], dtype="int32"),
                                                   mx.ndarray.array(neighbors_arr_up_lst[i], dtype="int32"),
                                                   mx.ndarray.array(centers_arr_lst[i]),
                                                   mx.ndarray.array(centers_mask_lst[i])]
                            else:
                                batch_data_lst += [mx.ndarray.array(neighbors_arr_lst[i], dtype="int32"),
                                                   mx.ndarray.array(neighbors_arr_up_lst[i], dtype="int32"),
                                                   mx.ndarray.array(neighbors_mask_up_lst[i]),
                                                   mx.ndarray.array(centers_arr_lst[i]),
                                                   mx.ndarray.array(centers_mask_lst[i])]
                    else:
                        if configs["up_type"] == "inter":
        ...
        return mx.io.DataBatch(data=batch_data_lst, label=batch_label)


class ScanNetWholeSceneLoader(BaseScanNetLoader):
    def __init__(self, root, batch_size=32, npoints=8192, split='train', shuffle=False, normalize=False,
                 augment_data=0, dropout_ratio=0, use_weights=False, use_cache=False, cache_file='', num_batch_limit=None):
        super(ScanNetWholeSceneLoader, self).__init__(root, batch_size, npoints, split, shuffle, normalize,
                                                      augment_data, dropout_ratio, use_weights, use_cache, cache_file)
        self.last_batch = None
        if num_batch_limit is not None:
            self.num_samples = min(num_batch_limit, self.num_samples)
    def reset(self):
    ...
    def _get_data_from_cache(self):
        if self.next_cache_index >= len(self.cache):
            return None
        nextbatch = self.cache[self.next_cache_index]
        self.next_cache_index += 1
        return nextbatch
    ...
    def _next_batch(self):
        if not self.use_cache or self.first_run:
            while self.buffer_label.shape[0] < self.batch_size:
                ...
                self.next_read_index += 1
            # get the next batch from buffer
            batch_data, self.buffer_data = self.buffer_data[:self.batch_size], self.buffer_data[self.batch_size:]
            batch_label, self.buffer_label = self.buffer_label[:self.batch_size], self.buffer_label[self.batch_size:]

            neighbors_arr_lst, neighbors_mask_lst, neighbors_arr_up_lst, neighbors_mask_up_lst, \
            centers_arr_lst, centers_mask_lst, centers_lst = \
                gridify.assemble_tensor(batch_data, voxel_size_lst=configs['voxel_size_lst'],
                                        grid_size_lst=configs['grid_size_lst'],
                                        lidar_coord=configs['lidar_coord'],
                                        max_p_grid_lst=configs['max_p_grid_lst'],
                                        max_o_grid_lst=configs['max_o_grid_lst'],
                                        kernel_size_lst=configs['kernel_size_lst'],
                                        stride_lst=configs['stride_lst'],
                                        single_padding_lst=configs['single_padding_lst'],
                                        para=configs["hash"] == "para",
                                        allow_sub=True, loc_weight=configs["loc_weight"],
                                        loc_within=configs["loc_within"],
                                        up_voxel_size_lst=configs['up_voxel_size_lst'],
                                        up_max_p_grid_lst=configs['up_max_p_grid_lst'],
                                        up_max_o_grid_lst=configs['up_max_o_grid_lst'],
                                        up_grid_size_lst=configs['up_grid_size_lst'],
                                        up_kernel_size_lst=configs['up_kernel_size_lst'],
                                        up_stride_lst=configs['up_stride_lst'])
            if self.use_cache:
                self.cache.append((batch_data, neighbors_arr_lst, neighbors_mask_lst, neighbors_arr_up_lst, neighbors_mask_up_lst, centers_arr_lst, centers_mask_lst, centers_lst, batch_label))
                print("append!!! batch:", str(self.next_read_index))
        else:
            if self.last_batch is None:
                ...
                if next_batch is None:
                    ...
                    batch_data, neighbors_arr_lst, neighbors_mask_lst, neighbors_arr_up_lst, neighbors_mask_up_lst, \
                        centers_arr_lst, centers_mask_lst, centers_lst, batch_label = next_batch
                    if self.batch_size < batch_data.shape[0]:
                        self.last_batch = [batch_data[self.batch_size:,...],
                                           [arr[self.batch_size:,...] for arr in neighbors_arr_lst],
                                           [arr[self.batch_size:,...] for arr in neighbors_mask_lst],
                                           [arr[self.batch_size:,...] for arr in neighbors_arr_up_lst],
                                           [arr[self.batch_size:,...] for arr in neighbors_mask_up_lst],
                                           [arr[self.batch_size:,...] for arr in centers_arr_lst],
                                           [arr[self.batch_size:,...] for arr in centers_mask_lst],centers_lst,
                                           batch_label[self.batch_size:,...]]
                        batch_data = batch_data[:self.batch_size, ...]
                        neighbors_arr_lst =[arr[:self.batch_size, ...] for arr in neighbors_arr_lst]
                        neighbors_mask_lst = [arr[:self.batch_size, ...] for arr in neighbors_mask_lst]
                        neighbors_arr_up_lst = [arr[:self.batch_size, ...] for arr in neighbors_arr_up_lst]
                        neighbors_mask_up_lst = [arr[:self.batch_size, ...] for arr in neighbors_mask_up_lst]
                        centers_arr_lst = [arr[:self.batch_size, ...] for arr in centers_arr_lst]
                        centers_mask_lst = [arr[:self.batch_size, ...] for arr in centers_mask_lst]
                        batch_label = batch_label[:self.batch_size, ...]
            else:
                batch_data, neighbors_arr_lst, neighbors_mask_lst, neighbors_arr_up_lst, neighbors_mask_up_lst, \
                centers_arr_lst, centers_mask_lst, centers_lst, batch_label = self.last_batch
                self.last_batch = None
