...
class BaseScanNetLoader(mx.io.DataIter):
    def __init__(self, root, batch_size=32, npoints=8192, split='train', shuffle=False, normalize="ball", augment_data=0, dropout_ratio=0, use_weights=False, use_cache=False, cache_file='', evel_only=False):
        self._augment_batch_data = [self._augment_batch_data_l0, self._augment_batch_data_l1, self._augment_batch_data_l2, self._augment_batch_data_l3, self._augment_batch_data_l4, self._augment_batch_data_l5,self._augment_batch_data_l6, self._augment_batch_data_l7]
        self.root = root
    ...
    def provide_data(self):
        if self.use_weights:
            return [('data', self.data_shape), ('weight', self.weight_shape), ('actual_centnum', (self.batch_size, 1), "int32")]
        else:
            return [('data', self.data_shape), ('actual_centnum', (self.batch_size, 1), "int32")]
    ...
    def _augment_batch_data_l6(self, batch_data, batch_label):
        batch_data = utils.rotate_point_cloud_z(batch_data)
        # batch_data = utils.rotate_perturbation_point_cloud(batch_data)
        #batch_data = utils.random_scale_point_cloud(batch_data)
        #batch_data = utils.shift_point_cloud(batch_data)
    ...
    def _augment_batch_data_l7(self, batch_data, batch_label):
        # batch_data = utils.rotate_point_cloud_z(batch_data)
        batch_data = utils.rotate_perturbation_point_cloud(batch_data)
        #batch_data = utils.random_scale_point_cloud(batch_data)
        #batch_data = utils.shift_point_cloud(batch_data)
...
class ScanNetLoader(BaseScanNetLoader):
    ...
    """
    def __init__(self, root, batch_size=32, npoints=8192, split='train', shuffle=False, normalize="ball", augment_data=0, dropout_ratio=0, use_weights=False, use_cache=False, cache_file='', evel_only=False):
        super(ScanNetLoader, self).__init__(root, batch_size, npoints, split, shuffle, normalize, augment_data, dropout_ratio, use_weights, use_cache, cache_file, evel_only)
...
class ScanNetWholeSceneLoader(BaseScanNetLoader):
    def __init__(self, root, batch_size=32, npoints=8192, split='train', shuffle=False, normalize="ball", augment_data=False, dropout_ratio=0, use_weights=False, use_cache=False, cache_file=''):
    ...
    def _next_batch(self):
        ...
        return mx.io.DataBatch(data=[mx.nd.array(data), mx.ndarray.ones((self.batch_size, 1), dtype='int32') * self.npoints], label=[mx.nd.array(label)])
