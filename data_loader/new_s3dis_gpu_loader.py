...
class S3DISLoader(mx.io.DataIter, threading.Thread):
    def __init__(self, filelist, batch_size=32, npoints=8192, split='train',shuffle=False, normalize="ball", augment_data=-1, use_weights=False, use_cache=False, cache_file='', qsize=64, max_epoch=200, drop_factor=0.0):
        ...
        self.bno = 0
        self.data_val = None
        self._augment_batch_data = [self._augment_batch_data_l1 , self._augment_batch_data_l1, self._augment_batch_data_l2, self._augment_batch_data_l3, self._augment_batch_data_l4, self._augment_batch_data_l5,self._augment_batch_data_l6, self._augment_batch_data_l7, self._augment_batch_data_l8, self._augment_batch_data_l9, self._augment_batch_data_l10, self._augment_batch_data_l11, self._augment_batch_data_l12, self._augment_batch_data_l13]

        is_list_of_h5_list = data_utils.is_h5_list(filelist)
        if is_list_of_h5_list:
            self.seg_list = [filelist]  # for train
        else:
            self.seg_list = data_utils.load_seg_list(filelist, split=split)  # for train
            self.mapping = data_utils.batch_mapping(self.seg_list)
    ...
    def sub_sample(self, batch_data, batch_label, batch_size, sample_num, point_nums_batch, max_nums, filter_extrema=2):
        ...
        for i in range(batch_size):
            ...
            if pt_num >= sample_num:
                ...
                choices = np.concatenate(np.random.shuffle(np.range(pt_num)),
                    np.array(random.sample(pt_num, sample_num - pt_num)))
    ...
    def work(self, epoch, file_idx_train):
        if not self.stopped:
            ...
            for batch_idx_train in range(batch_num):
                ...
                batch_dataxyz, batch_datafeat, points_num_batch, batch_label = self.sub_sample(batch_data, batch_label, self.batch_size, sample_num_wofset, points_num_batch, self.npoints, filter_extrema=2)
                # batch_data
    ...
    def onework(self, epoch, batch_counter):
        if not self.stopped:
            if batch_counter == 0 and self.shuffle:
                random.shuffle(self.batch_lst)
                self.file_pointer = 0
                print("seg_list reordered!")
            batch_data = []
            points_num_batch = []
            ...
            for i in range(self.batch_size):
                realidx = self.batch_lst[self.file_pointer]
                h5file, lineidx = self.seg_list[self.mapping[realidx,0]], self.mapping[realidx,1]
                data_pts, data_num, label = data_utils.load_onesample(h5file, lineidx)
                batch_data.append(data_pts)
                points_num_batch.append(data_num)
                batch_label.append(label)
                self.file_pointer+=1
            batch_data = np.concatenate(batch_data, axis=0)
            points_num_batch = np.concatenate(points_num_batch, axis=0)
            batch_label = np.concatenate(batch_label, axis=0)
            # print(batch_data.shape,points_num_batch.shape,batch_label.shape)
            offset = int(random.gauss(-self.npoints * self.drop_factor, self.npoints * self.drop_factor / 2))
            offset = max(offset, - 2 * self.npoints * self.drop_factor)
            offset = min(offset, 0)
            sample_num_wofset = int(self.npoints + offset)
            ...
            batch_label = [mx.ndarray.array(batch_label)]
            if not self.stopped:
                self.queue.put([mx.io.DataBatch(data=batch_data, label=batch_label), epoch, batch_counter])


    def run(self):
        if self.split == "train":
            self.batch_lst = list(range(self.mapping.shape[0]))
            print("sample numbers per epoch:", self.mapping.shape[0])
            self.file_pointer = 0
            self.num_batches = self.mapping.shape[0] // self.batch_size
            while self.bno < self.max_epoch * self.num_batches and not self.stopped:
                self.onework(self.bno // self.num_batches, self.bno % self.num_batches)
                self.bno += 1
        else:
...
if __name__ == "__main__":
    ...
    train_loader = S3DISLoader(
                '../data/3DIS/prepare_label_rgb/train_files_for_val_on_Area_5.txt',
                batch_size=1,
                npoints=8192,
