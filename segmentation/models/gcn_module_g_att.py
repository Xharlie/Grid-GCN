...
def verts_pair_func(ori_neighbor_feats, att_vec, B, max_o_grid, max_p_grid, N, C, att_full = "", pt_mlp_lst= None, contextvec = None,scope="pair_relation", bn_decay=0.9):
    '''
    :param centers:  B * max_o_grid * 3
    ...
    if configs["attfdim"] > 0:
        if BN:
            ...
            att_vec = mlp2d_c(att_vec, [C], use_bn=BN, bn_decay=bn_decay, attr=None, scope=scope + "/update_att_mlp2d_scnd")
        else:
            ...
            if contextvec is not None:
                att_vec = mx.sym.concat(att_vec, contextvec, dim=C_dim, name=scope + "/att_contextvec_concat")
                print("att_vec has contextvec")
            att_vec = fully_connected_mlp(att_vec, [C], dim=4, bn_decay=bn_decay, dropout_ratio=0, flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/update_att_mlp2dfc_scnd', flip=False)
...
def sub_g_update(centers_xyz, center_den, neighbors, has_feats, center_masks, neighbor_masks, attfdim,
        center_ori_feats = None, pt_mlp_lst = None, outDim= [64, 256], cntxt_mlp = None, shape = [64, 800000, 100, 6], scope='layer',
        aggtype="gcn", pool_type="max_pooling", att_full="", center_dim=[], recalden=False, bn_decay=0.9):
    '''
    ...
    if aggtype == "gcn":
        pair_feats = verts_pair_func(neighbor_feats, att_vec, B, max_o_grid,
            max_p_grid, N, C, contextvec=contextvec, att_full=att_full, pt_mlp_lst=pt_mlp_lst, scope=scope, bn_decay=bn_decay)  # B * max_o_grid * max_p_grid * C
