...
def update_func(centers, center_feats, outDim=[64, 256], scope="center_update"):
    ...
    if len(outDim) != 0:
        if BN:
            center_feats = mlp1d_c(center_feats, outDim, bn_decay=0.9, use_bn=BN,
                                   attr=None, scope=scope + '/cov1d')
        elif configs["use_bn"] == 'p':
            center_feats = fully_connected_mlp_withbn(center_feats, outDim, dim = 3, bn_decay=0.9, dropout_ratio=0,
                flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/cov1dfc', flip=False)
        else:
            center_feats = fully_connected_mlp(center_feats, outDim, dim = 3, bn_decay=0.9, dropout_ratio=0,
                flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/cov1dfc', flip=False)
    return center_feats
...
def aggregation_func(masked_pair_feats, neighbor_masks, geo_dist, max_o_grid, max_p_grid,
    ...
    if mlp_lst is not None:
        if BN:
            aggregated_feats = mlp1d_c(aggregated_feats, mlp_lst, bn_decay=0.9, use_bn=BN, attr=None,
                                           scope=scope + '/agg_mlp/cov1d')
        elif configs["use_bn"] == 'p':
            aggregated_feats = fully_connected_mlp_withbn(aggregated_feats, mlp_lst, dim=3, bn_decay=0.9, dropout_ratio=0,
                flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/agg_fc', flip=False)
        else:
            aggregated_feats = fully_connected_mlp(aggregated_feats, mlp_lst, dim = 3, bn_decay=0.9, dropout_ratio=0, flatten=False, use_bn=BN,
...
def interpolate_all(dist, masked_pair_feats, neighbor_masks, scope=''):
    ...
    norm = mx.symbol.sum(weight, axis=3, keepdims=True)   # (B, O, 1)
    weight = mx.symbol.broadcast_div(weight, norm).expand_dims(axis=1)  # (B, 1, O, P)
    interp_feature = mx.symbol.broadcast_mul(masked_pair_feats, weight)  # (B, C, O, P)
    interp_feature = mx.symbol.sum(interp_feature, axis=3)  # (B, O, C)
...
def neighbors_fetch(neighbors, neighbor_masks, centers, shape, k=3, scope=''):
    ...
    """
    B, O, P, C = shape
    neighbor_locs = mx.sym.slice_axis(neighbors, axis=C_dim, begin=0, end=3)  # shape=(B, 3, O, P)
    centers_expand_xyz = mx.sym.slice_axis(mx.sym.expand_dims(centers, axis=P_dim), axis=C_dim, begin=0, end=3) # shape=(B, 3, O, 1)
    diff = mx.symbol.broadcast_sub(neighbor_locs, centers_expand_xyz)  # (B, 3, O, P)
    all_dist = mx.symbol.sum(diff*diff, axis=C_dim) + (1.0 - neighbor_masks) # (B, O, P)
...
def verts_pair_func(centers, neighbor_feats, att_vec, B, max_o_grid, max_p_grid, N, C, pt_mlp_lst= None, scope="pair_relation"):
    ...
    if pt_mlp_lst is not None and len(pt_mlp_lst) > 0:
        ...
        if BN:
            neighbor_feats = mlp2d_c(neighbor_feats, pt_mlp_lst, use_bn=BN, bn_decay=configs["bn_decay"], scope=scope)
        else:
            neighbor_feats = fully_connected_mlp(neighbor_feats, pt_mlp_lst, dim = 4, bn_decay=configs["bn_decay"], dropout_ratio=0,
                flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/cov2fc', flip=False)
    # centers=centers.reshape((B, max_o_grid, 3), name=scope+"/centerreshape")
    if configs["attfdim"] > 0:
        if BN:
            att_vec = mlp2d_c(att_vec, [C//4, C], use_bn=BN, bn_decay=0.9, attr=None, scope=scope+"/update_att_mlp2d")
        else:
            att_vec = fully_connected_mlp(att_vec, [C//4, C], dim = 4, bn_decay=configs["bn_decay"], dropout_ratio=0,
                flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/update_att_mlp2dfc', flip=False)
...
def sub_g_update(centers, neighbors, has_feats, center_masks, neighbor_masks, attfdim,
        ...
        no_mask=False, aggtype="gcn", pool_type="max_pooling", neighbor_num = 0, att_full=False, center_dim=[]):
    '''
    ...
    centers_expand = mx.sym.tile(mx.sym.expand_dims(centers, axis=P_dim, name=scope+"/centerexpand"), reps=(1, 1, 1, max_p_grid) if BN else (1, 1, max_p_grid, 1))
    centers_expand_xyz = mx.sym.slice_axis(centers_expand, axis=C_dim, begin=0, end=3)
    ...
    if max(attfdim, configs["localfdim"]) in [4,5,11,12]:
        ...
        geo_den_sum = mx.sym.sum(geo_num, axis=C_dim, keepdims=True)  # B * max_o_grid * 1
        # geo_den_sum = mx.sym.slice_axis(centers_expand, axis=C_dim, begin=3, end=4) # B * 1 * O * P or B * O * P * 1
        # if center_masks is not None:
        #     center_masks_expands = mx.sym.tile(mx.sym.expand_dims(center_masks.expand_dims(axis=1 if BN else 2), axis=P_dim, name=scope+"/centermaskexpand"), reps=(1, 1, 1, max_p_grid) if BN else (1, 1, max_p_grid, 1))
        #     geo_den_sum = geo_den_sum + 1-center_masks_expands
    ...
    if attfdim <= 3:
        ...
        att_vec = mx.sym.concat(geo_vec, geo_global_density, dim=C_dim, name=scope+"/attconcat")
    elif attfdim == 5:
        att_vec = mx.sym.concat(geo_vec, geo_density, geo_global_density, dim=C_dim, name=scope+"/attconcat")
    elif attfdim == 10:
    ...
    if configs["localfdim"] <= 3:
        ...
        geo_feats =  mx.sym.concat(geo_vec, geo_global_density, dim=C_dim, name=scope+"/geo_concat")
    elif configs["localfdim"] == 5:
        geo_feats = mx.sym.concat(geo_vec, geo_density, geo_global_density, dim=C_dim, name=scope+"/geo_concat")
    ...
    # neighbor_feats = mx.symbol.concat(geo_feats, neighbors, dim=C_dim, name=scope+"/neighbor_feats_concat")
    ...
    if aggtype == "gcn":
        ...
        pair_feats = verts_pair_func(centers, neighbor_feats, att_vec,
                                     B, max_o_grid, max_p_grid, N, C, pt_mlp_lst=pt_mlp_lst,
                                     scope=scope)  # B * max_o_grid * max_p_grid * C
        ...
        agg_neigh_feats = aggregation_func(pair_feats, neighbor_masks, geo_dist, max_o_grid, max_p_grid, pool_type=pool_type, scope=scope+"/center_aggregation") # B * max_o_grid * 1 * outDim[-1]
    elif aggtype == "agg_gcn":
        agg_neigh_feats = aggregation_func(neighbor_feats, neighbor_masks, geo_dist, max_o_grid, max_p_grid,
            pool_type=pool_type, mlp_lst=pt_mlp_lst, scope=scope + "/neigh_aggregation")
    else:
        raise NotImplementedError("aggtype %s is not implemented" % aggtype)
    if center_ori_feats is not None:
        if len(center_dim) > 0:
            if BN:
                center_feats = mlp1d_c(center_ori_feats, center_dim, bn_decay=0.9, use_bn=BN, attr=None, scope=scope + '/centerfeats_conv1d')
            elif configs["use_bn"] == 'p':
                center_feats = fully_connected_mlp_withbn(center_ori_feats, center_dim, dim=3, bn_decay=0.9, dropout_ratio=0,
                                                   flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None,
                                                   scope=scope + '/centerfeats_conv1d1d', flip=False)
            else:
                center_feats = fully_connected_mlp(center_ori_feats, center_dim, dim = 3, bn_decay=0.9, dropout_ratio=0,
                   flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/centerfeats_conv1d1d', flip=False)
        else:
    ...
    agg_feats = update_func(centers, agg_neigh_feats, outDim=outDim, scope=scope + "/center_update")  # B * max_o_grid * outDim[-1]
