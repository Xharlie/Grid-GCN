...
def sampling(data, shape, method='fps', fps_npoints=0, scope=''):
    ...
    if method == 'fps':
        ...
        centeroids_ids = mx.sym.contrib.FarthestPointSampling(data=data, npoints=fps_npoints, name=scope+'/fps_idx')  # (B, fps_npoints)
    elif method == 'random':
...
def grouping(data, feature, centeroids, shape, fps_npoints, rs_npoints, radius=0, xyz_mlp=[], method='rs', knn_dilation=1, scope='', conxyz=True):
    ...
    if method == 'rs':
        ...
        grouped_ids = mx.sym.contrib.BallQuery(data, centeroids, radius=radius, nsample=rs_npoints) # (B, fps_npoints, rs_npoints)
    elif method == 'knn':
...
def interpolate(query, data, feature, shape, k=3, scope=''):
    ...
    if k == 3:
        ...
        dist, ids = mx.sym.contrib.ThreeNN(query, data)  # (B, M, k)
    else:
        dist, ids = knn(query, data, (M,N), k)  # (B, M, k)
