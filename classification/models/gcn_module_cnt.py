...
def sub_g_update(max_p_grid, cnt_ratio, centers_xyz, center_den, neighbors, cntneighbors, has_feats, center_masks, neighbor_masks, attfdim, pt_mlp_lst = None, att_ele_lst = None, outDim= [64, 256], cntxt_mlp = [], shape = [64, 800000, 100, 6], scope='layer', aggtype="gcn", pool_type="max_pooling", att_full="off", recalden=False, bn_decay=0.9):
    '''
    ...
    if not has_feats:
        cntneighbors = None
    contextvec = None
    if cntxt_mlp is not None and cntneighbors is not None:
        contextvec = contextvec_func(cntneighbors, max_p_grid, cnt_ratio, cntxt_mlp = cntxt_mlp, bn_decay=bn_decay, scope=scope)
...
def contextvec_func(cntxt, max_p_grid, cnt_ratio, cntxt_mlp = [64,64], bn_decay=0.9, scope="", cnt_mode=configs["cnt_mode"]):
    print("cntxt_mlp:", cntxt_mlp, cnt_mode)

    ele_type, pooling_type = cnt_mode.split("_")

    if len(cntxt_mlp)>0 and ele_type == "elefrst":
        if BN:
    ...
    aggregated_cntxt = mx.sym.Pooling(name=scope + "/cnt/max_pooling", data=cntxt,
                                      kernel=(1, max_p_grid*cnt_ratio) if BN else (max_p_grid*cnt_ratio, 1), pool_type=pooling_type, layout="NCHW")
    aggregated_cntxt = mx.sym.tile(aggregated_cntxt, reps=(1, 1, 1, max_p_grid) if BN else (1, 1, max_p_grid, 1))

    if len(cntxt_mlp)>0 and ele_type == "elelst":
        if BN:
            cntxt = mlp1d_c(cntxt, cntxt_mlp, bn_decay=bn_decay, use_bn=BN,
                                   attr=None, scope=scope+"/cnt")
        else:
            cntxt = fully_connected_mlp(cntxt, cntxt_mlp, dim=3, bn_decay=bn_decay, dropout_ratio=0,
                                               flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None,
                                               scope=scope + '/cnt/cov1dfc', flip=False)
