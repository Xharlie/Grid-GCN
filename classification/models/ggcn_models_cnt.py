...
from models.gcn_module_cnt import sub_g_update
from utils.ops import fully_connected, batch_take, batch_take_g, batch_take_g_cnt
    ...
    for i in range(len(configs["voxel_size_lst"])):
        ...
        if configs["group_all"] and i == len(configs["voxel_size_lst"])-1:
            ...
            if BN:
                ...
                neighbors, cntneighbors= batch_take_g_cnt(data, nebidx, take_shapes[i],
                    scope='down_neigh_batch_take_'+str(i), cnt_ratio=configs["cnt_ratios"][i])
                neighbors = neighbors.transpose(axes=(0, 3, 1, 2), name="/down_batch_take_g_post_tpose{}".format(i))
                if configs["cnt_ratios"][i] == 1:
                    cntneighbors = neighbors
                else:
                    cntneighbors = mx.sym.slice_axis(cntneighbors, axis=-1, begin=4, end=None)
                    F = cntneighbors.transpose(axes=(0, 3, 1, 2),
                                name="/down_batch_take_g_cnt_post_tpose{}".format(i))
