...
def update_func(centers, center_feats, outDim=[64, 256], scope="center_update"):
    ...
    for i in range(len(outDim)):
        center_feats = fully_connected(center_feats, outDim[i], bn_decay=0.9, dropout_ratio=0,
...
def aggregation_func(masked_pair_feats, max_o_grid, max_p_grid,
                     pool_type = "max", mlp = None, scope='center_aggregation'):
    '''
    ...
    if pool_type == "max_pooling":
        ...
        return mx.sym.squeeze(mx.symbol.transpose(aggregated_feats, axes=(0,2,3,1)), axis=2)
    else:
...
def sub_g_update(centers, neighbor_feats, neighbor_locs, center_masks, neighbor_masks, pt_mlp_lst = None,
         outDim= [64, 256], shape = [64, 800000, 100, 6], scope='layer', no_mask=False, aggtype="gcn"):
    '''
    ...
    neighbor_locs_xyz = mx.sym.slice_axis(neighbor_locs, axis=3, begin=0, end=3)
    geo_vec = neighbor_locs_xyz - centers_expand_xyz # B * max_o_grid * 3
    if max(configs["attfdim"], configs["localfdim"]) in [4,5,11,12]:
        geo_num = mx.sym.slice_axis(neighbor_locs, axis=3, begin=3, end=4)
    ...
    if configs["attfdim"] <= 3:
        ...
        geo_dist = mx.sym.sqrt(mx.sym.sum(mx.sym.square(geo_vec), axis=3)).expand_dims(axis=3)
        att_vec = mx.sym.concat(geo_dist, geo_vec, centers_expand_xyz, neighbor_locs_xyz, dim=3)
    elif configs["attfdim"] == 11:
        geo_dist = mx.sym.sqrt(mx.sym.sum(mx.sym.square(geo_vec), axis=3)).expand_dims(axis=3)
        att_vec = mx.sym.concat(geo_dist, geo_vec, centers_expand_xyz, neighbor_locs_xyz, geo_density, dim=3)
    elif configs["attfdim"] == 12:
        geo_dist = mx.sym.sqrt(mx.sym.sum(mx.sym.square(geo_vec), axis=3)).expand_dims(axis=3)
        att_vec = mx.sym.concat(geo_dist, geo_vec, centers_expand_xyz, neighbor_locs_xyz, geo_density, geo_global_density, dim=3)
    else:
    ...
    if configs["localfdim"] <= 3:
        ...
        geo_dist = mx.sym.sqrt(mx.sym.sum(mx.sym.square(geo_vec), axis=3)).expand_dims(axis=3)
        geo_feats = mx.sym.concat(geo_dist, geo_vec, centers_expand_xyz, neighbor_locs_xyz, dim=3)
    elif configs["localfdim"] == 11:
        geo_dist = mx.sym.sqrt(mx.sym.sum(mx.sym.square(geo_vec), axis=3)).expand_dims(axis=3)
        geo_feats = mx.sym.concat(geo_dist, geo_vec, centers_expand_xyz, neighbor_locs_xyz, geo_density, dim=3)
    elif configs["localfdim"] == 12:
        geo_dist = mx.sym.sqrt(mx.sym.sum(mx.sym.square(geo_vec), axis=3)).expand_dims(axis=3)
        geo_feats = mx.sym.concat(geo_dist, geo_vec, centers_expand_xyz, neighbor_locs_xyz, geo_density, geo_global_density, dim=3)
    else:
        raise NotImplementedError
    ...
    if aggtype == "gcn":
        ...
        if not no_mask and pool_type!="max" and pool_type!="max_pooling":
            pair_feats = mx.sym.broadcast_mul(pair_feats, mx.sym.expand_dims(neighbor_masks, axis=3), name="pairmask") # B * max_o_grid * max_p_grid * C
        center_feats = aggregation_func(pair_feats, max_o_grid, max_p_grid, pool_type=pool_type, scope=scope+"/center_aggregation") # B * max_o_grid * 1 * outDim[-1]
        center_feats = update_func(centers, center_feats, outDim=outDim, scope=scope+"/center_update") # B * max_o_grid * outDim[-1]
        # center_feats = center_feats.reshape((B, max_o_grid, outDim[-1]), name=scope+"/center_feats_reshape")
        if not no_mask:
        ...
        center_feats = mlp_mask_and_pool(neighbor_feats, pt_mlp_lst, max_p_grid, pool_type=pool_type,
            use_bn=True, mask=mx.sym.expand_dims(neighbor_masks, axis=3), scope='{}/{}'.format(scope, "mlp_mask_and_pool"))
        if not no_mask:
            center_feats = mx.sym.broadcast_mul(center_feats, mx.sym.expand_dims(center_masks,axis=2), name="centermask") # B * max_o_grid * outDim[-1]
    else:
