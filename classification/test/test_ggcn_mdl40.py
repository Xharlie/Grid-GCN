...
from data_loader.ggcn_modelnet40_loader import ModelNet40Loader
from ggcn_models import get_symbol_cls_ggcn, get_symbol_cls_fps_gcn
...
if __name__ == "__main__":
    ...
    if configs['task'] == 'cls':
        if configs['fps']:
            symbol = get_symbol_cls_fps_gcn(configs['batch_size'], configs['num_points'],
                                                 take_shapes, gcn_outDim=configs['gcn_outDim'])
        else:
            symbol = get_symbol_cls_ggcn(configs['batch_size'], configs['num_points'],
                                                 take_shapes, gcn_outDim=configs['gcn_outDim'])
    if configs['use_cpu']:
        ctx = mx.cpu()
    else:
    ...
    for rot in np.linspace(0, np.pi*2, NUM_VOTES, endpoint=False):
        ...
        for i, batch in enumerate(val_loader):
            ...
            # print(module.get_outputs()[0].asnumpy()[0,...])
