...
def load_seg_list(filelists, split="test"):
    folder = os.path.dirname(filelists)
    filelist =[os.path.join(folder, line.strip()) for line in open(filelists)]
    if split=="test":
        return filelist
    else:
        h5files = []
        for flist in filelist:
            filefolder = os.path.dirname(flist)
            for line in open(flist):
                h5files.append(os.path.join(filefolder, line.strip()))
    return h5files

def batch_mapping(h5files):
    mapping = []
    for fidx, h5file in enumerate(h5files):
        with h5py.File(h5file, 'r') as data:
                nums=data['label_seg'].len()
        for i in range(nums):
            mapping.append([fidx, i])        
...
def load_onesample(h5file, idx):
        with h5py.File(h5file, 'r') as data:
            points=data['data'][[idx],...].astype(np.float32)
            point_nums=data['data_num'][[idx],...].astype(np.int32)
            labels_seg=data['label_seg'][[idx],...].astype(np.int32)   
            return points, point_nums, labels_seg
...
def filter_extre(dataxyz, datafeat, label, filter_extrema):
    data_x = dataxyz[:,0]
    data_y = dataxyz[:,1]
    data_z = dataxyz[:,2]
    # indx = (np.abs(data_x-np.median(data_x)) < filter_extrema) & (np.abs(data_y-np.median(data_y)) < filter_extrema) & (np.abs(data_z-np.median(data_z)) < filter_extrema)
    indx = (np.abs(data_x-np.mean(data_x)) < filter_extrema) & (np.abs(data_z-np.mean(data_z)) < filter_extrema)
    dataxyz_return =  dataxyz[indx,:] 
    datafeat_return = datafeat[indx,:] if datafeat is not None else None
    label_return = label[indx] if label is not None else None
    return dataxyz_return, datafeat_return, label_return


def filter_extre_indx(dataxyz, filter_extrema):
    data_x = dataxyz[:,0]
    data_y = dataxyz[:,1]
    data_z = dataxyz[:,2]
    truefalse = (np.abs(data_x-np.mean(data_x)) < filter_extrema) & (np.abs(data_z-np.mean(data_z)) < filter_extrema)
    indx = np.where(truefalse)
    return indx[0], indx[0].shape[0]