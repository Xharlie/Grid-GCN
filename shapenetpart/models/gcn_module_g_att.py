...
def verts_pair_func(ori_neighbor_feats, att_vec, B, max_o_grid, max_p_grid, N, C, att_full = "", pt_mlp_lst= None, contextvec = None,scope="pair_relation", bn_decay=0.9, neighbor_masks_expand=None, att_norm=None):
    ...
    if configs["attfdim"] > 0:
        if BN:
            att_vec = mlp2d_c(att_vec, [C], use_bn=BN, bn_decay=bn_decay, attr=None,
            ...
            att_vec = mlp2d_c(att_vec, [C, C] if contextvec is not None else [C], use_bn=BN, bn_decay=bn_decay, attr=None, scope=scope + "/update_att_mlp2d_scnd")
        else:
            att_vec = fully_connected_mlp(att_vec, [C], dim=4, bn_decay=bn_decay, dropout_ratio=0, flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None, scope=scope + '/update_att_mlp2dfc_frst', flip=False)
            if att_full == "last":
                att_vec = mx.sym.concat(att_vec, ori_neighbor_feats, dim=C_dim, name=scope + "/att_full_concat")
                print("has attfull last")
            elif att_full == "next":
...
def sub_g_update(centers_xyz, center_den, neighbors, has_feats, center_masks, neighbor_masks, attfdim,
    ...
    if aggtype == "gcn":
        pair_feats = verts_pair_func(neighbor_feats, att_vec, B, max_o_grid,
            max_p_grid, N, C, contextvec=contextvec, att_full=att_full, pt_mlp_lst=pt_mlp_lst, scope=scope, bn_decay=bn_decay, neighbor_masks_expand=neighbor_masks_expand, att_norm=att_norm)  # B * max_o_grid * max_p_grid * C
    ...
    if center_ori_feats is not None:
        if len(center_dim) > 0:
            if BN:
                ...
                center_feats = fully_connected_mlp_withbn(center_ori_feats, center_dim, dim=3, bn_decay=bn_decay, dropout_ratio=0,flatten=False, use_bn=BN, use_relu=True, bn_axis=1, attr=None,
